...
from dbgpt.configs.model_config import (EMBEDDING_MODEL_CONFIG,LLM_MODEL_CONFIG,LOGDIR,STATIC_MESSAGE_IMG_PATH,)


# 项目路径 / 根目录
ROOT_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # '/home/a864a/DB-GPT'
...
# WEB UI 设置
static_file_path = os.path.join(ROOT_PATH, "dbgpt", "app/static/web") if CFG.USE_NEW_WEB_UI \
    else os.path.join(ROOT_PATH, "dbgpt", "app/static/old_web")


from dbgpt.app.base import (
    WebServerParameters, # cls
    _create_model_start_listener, # func
    _migration_db_storage, # func
    server_init # func
)
...
app = create_app(title=_("DB-GPT Open API"), description=_("DB-GPT Open API"), version=version, openapi_tags=[])
replace_router(app) # # Use custom router to support priority
...
def mount_routers(app: FastAPI):
    ...
    from dbgpt.serve.agent.app.recommend_question.controller import router as recommend_question_v1
...
def mount_static_files(app: FastAPI):
    ...
    app.mount("/swagger_static",StaticFiles(directory=static_file_path),name="swagger_static",)
...
def _get_webserver_params(args: List[str] = None):
    ...
    webserver_params: WebServerParameters = parser.parse_args_into_dataclass(WebServerParameters,env_prefixes=[env_prefix],command_args=args,)
...
def initialize_app(param: WebServerParameters = None, args: List[str] = None):
    ...
    如果您使用 gunicorn 作为进程管理器，那么可以在‘ on _ start ’钩子中调用 initialize _ app。
    Args:
    ...
    if not param.log_level:
        from dbgpt.util.utils import _get_logging_level
        param.log_level = _get_logging_level()
    
    from dbgpt.util.utils import setup_logging
    setup_logging("dbgpt", logging_level=param.log_level, logger_filename=param.log_file)
    ...
    if rerank_model_name:
        rerank_model_path = CFG.RERANK_MODEL_PATH or EMBEDDING_MODEL_CONFIG.get(rerank_model_name)
...
def run_uvicorn(param: WebServerParameters):
    import uvicorn # 运行 Python 的异步 Web 框架，比如 FastAPI 和 Starlette。它支持异步和并发操作
    from dbgpt.util.utils import setup_http_service_logging
    ...
    from dbgpt.util.utils import logging_str_to_uvicorn_level
    uvicorn.run(cors_app,host=param.host,port=param.port,log_level=logging_str_to_uvicorn_level(param.log_level),)
...
def run_webserver(param: WebServerParameters = None):
    ...
    with root_tracer.start_span(
        ...
        # # TODO
        # from dbgpt.serve.agent.agents.expand.app_start_assisant_agent import (StartAppAssistantAgent,)  # noqa: F401
        # from dbgpt.serve.agent.agents.expand.intent_recognition_agent import (IntentRecognitionAgent,)  # noqa: F401
